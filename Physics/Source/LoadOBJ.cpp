...
bool LoadOBJ(const char* file_path, std::vector<Position>& out_vertices, std::vector<TexCoords>& out_uvs, std::vector<Vector3>& out_normals){
	...
	for(unsigned i = 0; i < vertexIndices.size(); ++i){ //For each vertex of each triangle
		...
		TexCoords uv = temp_uvs[uvIndex - 1];
...
void IndexVBO(std::vector<Position>& in_vertices, std::vector<TexCoords>& in_uvs, std::vector<Vector3>& in_normals, std::vector<unsigned>& out_indices, std::vector<Vertex>& out_vertices){
	std::map<PackedVertex,unsigned short> VertexToOutIndex;
	for(unsigned int i = 0; i < in_vertices.size(); ++i){ //For each input vertex
		...
		if(found){ 
			out_indices.push_back(index); //A similar vertex is alr in the VBO so use it instead
		} else{ //If not, it needs to be added in the output data
			Vertex v;
			v.pos.Set(in_vertices[i].x, in_vertices[i].y, in_vertices[i].z);
			v.texCoords.Set(in_uvs[i].u, in_uvs[i].v);
